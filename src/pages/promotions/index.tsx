...
type ColumnType = ColumnDef<PromotionInterface, unknown>;
...
export function PromotionListPage(props: PromotionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PromotionInterface>>(
    () => `/promotions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PromotionInterface) => {
    if (hasAccess('promotion', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/promotions/view/${row.id}`);
    }
  };
