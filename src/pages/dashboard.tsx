...
import { SessionListPage as ShowCaseList } from 'pages/sessions';
...
function DashboardPage() {
  ...
  const sessionStat = useSWR<PaginatedInterface<SessionInterface>>(
    () => `/sessions`,
    () => getSessions({ limit: 1, offset: 0 }),
  );

  const promotionStat = useSWR<PaginatedInterface<PromotionInterface>>(
    () => `/promotions`,
    () => getPromotions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Sessions',
      result: sessionStat?.data?.totalCount,
      isLoading: sessionStat.isLoading,
    },
    {
      title: 'Promotions',
      result: promotionStat?.data?.totalCount,
      isLoading: promotionStat.isLoading,
    },
  ];
